...
title_footer.initialize=function(background)
{
    ...
    Reveal.addEventListener('slidechanged',function(event){
        var slideTitle=event.currentSlide.getElementsByClassName("slide-title");
        var footer=document.getElementById("title-footer-p");
        for (var i=0;i<footer.children.length;i++)
            footer.removeChild(footer.children[i]);
        if (slideTitle.length>0)
        {
            footer.appendChild(slideTitle[0].cloneNode(true));
        }
    });
	...
	title_footer_p.setAttribute('id','title-footer-p');
    ...
    // Update for the first slide.

    var slideTitle=Reveal.getCurrentSlide().getElementsByClassName("slide-title");
    var footer=document.getElementById("title-footer-p");
    for (var i=0;i<footer.children.length;i++)
        footer.removeChild(footer.children[i]);
    if (slideTitle.length>0)
